...
<?php 
        if ( !is_home() ) { ?>
            jQuery(document).ready(function($) {  	   
                ...
                jQuery('#nav a[href^="<?php echo site_url(); ?>"]:not(.separate)').each(function(){
	...
	jQuery(document).ready(function($) {  
		...
		jQuery('#nav a[href^="<?php echo site_url(); ?>"]:not(.separate)').click(function (e) {  
                        e.preventDefault();
			jQuery.scrollTo.window().queue([]).stop(); // Prevent scroll queue from building up
